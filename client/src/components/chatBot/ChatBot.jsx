...
function ChatBot({ projetId, isFixed }) {
  ...
  useEffect(() => {
    ...
    // close the chatbot if the user clicks outside
  ...
  //automatically scroll to bottom if a new message is generated
  ...
  // toggle the visibility of the chat when clicking on the chatbot icon
  const toggleVisiblity = () => {
    setIsVisible(prev => !prev);
  };


  // get the answer when a query is sent
  ...
  return (
    <div ref={wrapperRef} className={`fixed right-4 bottom-4 transition-all duration-300 z-10000`}>
      ...
      {//if the chat is open show the conversation
        isVisible && (
          <div className="relative w-[40vw] h-[90vh] bg-[color:var(--color-neutral-100)] rounded-[var(--border-radius-8)] flex flex-col overflow-hidden shadow-xl">
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-[color:var(--color-neutral-100)] flex flex-col gap-3">
              ...
              {messages.map((msg, index) => (
                <div
                  key={index}
                  className={`max-w-[80%] p-3 rounded-[var(--border-radius-8)] text-justify font-[400] text-[1.125rem] ${msg.role === 'user'
      ...
      {// if the chat is closed show the chatbot icon 
        !isVisible && (
          <Tooltip element={
            <button
              onClick={toggleVisiblity}
