...
router.put("/editable/:sectionId", validateToken, async (req, res) => {
    try {
...
router.get("/editable/:sectionId", validateToken, async (req, res) => {
    try {
        ...
        // Fetch the section with all necessary relationships
        const section = await sectionModel.findById(sectionId)
            .populate({
                path: "annotations",
                populate: { path: "auteur" } // Populate annotation author
            })
            .populate({
                path: "conflits",
                match: { resolu: false, valide: true  },
                populate: { path: "signaleur" } // Populate conflict reporter
            })
            .populate({
                path: "projetId",
                populate: [
                    { path: "chef" }, // Populate project leader
                    { path: "collaborateurs" } // Populate project collaborators
                ]
            });
        ...
        return res.status(200).json({
            section,
            userEditing, // Collaborateur editing the section
            userChef, // Project leader
            projet, // Full project details
